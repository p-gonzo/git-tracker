  ...
  render() {
    return (
      <div className="card small-text" onClick={() => this.setState({isExpanded: !this.state.isExpanded})}>
        <span className="bold">{ this.props.data.author }</span> committed at <span className="bold">{ this.props.data.created_at }</span> with the following message: <span className="italic">{ this.props.data.title }</span>
